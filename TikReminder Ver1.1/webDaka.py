...
def getCookies():
    ...
    element_keyword.send_keys('#uia系统账号#')
    element_keyword = driver.find_element_by_id('password')
    element_keyword.send_keys('#uia系统密码#')
...
def getUnsignedList(Request_Cookies):
    ...
    data = {"page": 1,"rows": 800, "sort": "bj", "order": "asc"}
...
def Doit(urgent = False):
    ...
    for i in range(len(wtblist)):
        ...
        #print(bj, xm, xh, dh)
        classIndex = -1
        for j in range(len(classContainer)):
            if bj == classContainer[j]["class"]:
                classIndex = j
    
        if not classIndex == -1:
            classContainer[classIndex]["member"].append({"name": xm, "stdId": xh, "phone": dh})
        else:
            classContainer.append({"class":bj,"member":[{"name": xm, "stdId": xh, "phone": dh}]})
    classContainer.sort(key = lambda x:x["class"])
    
    config = configparser.ConfigParser()
    config.read_file(open('config\config.ini', encoding="utf-8"))    
    if not int(config.get("状态","全部完成")):
        allClaFinished = CoolQ.sendUnsignedNotice(classContainer, urgent)
        if allClaFinished:
            config.set("状态", "全部完成", "1")
            config.write(open('config\config.ini', "w", encoding="utf-8"))
    
    for k in range(len(classContainer)):
        className = classContainer[k]["class"]
        CoolQ.sendMyClassUnsignedNotice(className, classContainer[k]["member"])
    #print(messageList)
